...
The pilot data has a known HALT (BD-12843443-4 Fillering null proc_time, the
locked Section 9 #4 finding). To exercise the *full* pipeline end-to-end we
patch a placeholder proc_time for that one row before kicking off bootstrap.

Two scenarios:

  1. **HALT path** — vanilla pilot inputs. Audit HALTs at code 10; downstream
     routes don't run; schedule.csv is NOT written.

  2. **Full-pipeline path** — same inputs with BD Fillering proc_time
     synthetically set to 60 SEC/BATCH. All 12 routes run; every Section 11
     artefact lands in the run folder. Then we re-run and assert the
     deterministic artefacts (schedule.csv, machine_view.csv, kpi.csv,
     dag.json, reservation_log.csv, aging_violations.csv,
     building_to_curing.csv, infeasibilities.csv, routing_cleaned.csv) are
     byte-identical across the two runs (L11 / Section 12.2).
"""
...
def _patch_bd_fillering_proc_time(
    src_dir: Path, dst_dir: Path, proc_time_sec: float = 60.0
) -> None:
    """Copy the input folder, then write a `proc_time` value into the
    `BD-12843443-4` Fillering row of the routing sheet."""
...
class TestHaltPath:
    def test_pilot_inputs_halt_at_audit(
        ...
        assert len(runs) == 1
        run_dir = runs[0]
        # audit artefacts present; downstream artefacts absent.
...
class TestFullPath:
    ...
    def patched_inputs(self, project_root: Path, tmp_path_factory) -> Path:
        ...
        _patch_bd_fillering_proc_time(src, dst)
        return dst
